...
class KDTree
{
  ...
  private:
     ...
     */
     ...
     Point<Dim> quickselect(vector<Point<Dim>>& list, int dim, unsigned left, unsigned right, unsigned pivotIndex);
     KDTreeNode* ctor(vector<Point<Dim>>& points,int dim, unsigned left, unsigned right);
     Point<Dim> findNearestNeighbor(const Point<Dim>& query, typename KDTree<Dim>::KDTreeNode* currentNode, int currDim) const;
     void clear(KDTreeNode* subroot);
     void copy(KDTreeNode* subroot, KDTreeNode* other_subroot);
