 ...
 */
 ...
 size_t binary_search(const std::vector<T>& elements, const C& val, int l, int r) {
   ...
   if (elements[mid] > val) {
     return binary_search(elements, val, 0, mid);
   }

   if (elements[mid] < val) {
     return binary_search(elements,val, mid+1, r);
   }
   return -1;
...
size_t insertion_idx(const std::vector<T>& elements, const C& val)
{
    ...
    return binary_search(elements, val, 0, elements.size()-1);
}
