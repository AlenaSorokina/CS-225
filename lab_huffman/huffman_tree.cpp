...
HuffmanTree::removeSmallest(queue<TreeNode*>& singleQueue,
    ...
    if (singleQueue.empty()) {
      //remove first element from merege queue
      temp = mergeQueue.front();
      mergeQueue.pop();
      return temp;
    }
    if (mergeQueue.empty()) {
      //remove first element in the single queue
...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while(!(singleQueue.empty()) || mergeQueue.size()!=1) {
      ...
      TreeNode* intern = new TreeNode(temp1->freq.getFrequency()+temp2->freq.getFrequency());
      intern->left = temp1;
      intern->right = temp2;
      mergeQueue.push(intern);
    }
