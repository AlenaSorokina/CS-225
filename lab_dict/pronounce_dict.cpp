...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    string temp_word2 = word2;

    std::transform(temp_word1.begin(), temp_word1.end(), temp_word1.begin(), ::toupper);
    std::transform(temp_word2.begin(), temp_word2.end(), temp_word2.begin(), ::toupper);

    if(dict.count(temp_word1) ==0 || dict.count(temp_word2)==0) return false;

    std::vector<string> pr1 = dict.at(temp_word1);
    std::vector<string> pr2 = dict.at(temp_word2);

    if(pr1.size() != pr2.size()) return false;

    for(unsigned i = 0; i<pr1.size(); i++){
      if(pr1[i]!=pr2[i]) {
        return false;
      }
    }
